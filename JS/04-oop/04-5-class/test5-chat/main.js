...
class Emoji {
    ...
    //클래스의 static과 생성자 함수의 prototype은 다르다
    //객체 생성의 메모리에 매번 할당 되지 않는다는 공통점이 있지만 이용 및 목적이 다르다
    //prototype은 객체로 이용
    //static은 객체로 이용 불가. 클래스명으로 이용한다

    //클래스 내에 함수를 선언할때 선언식으로 추가하거나 표현식으로 추가한다
    //선언식으로 추가하면 내부적으로 prototype에 , 추가를 한다.
    //표현식으로 추가하면 객체 메모리에 추가한다.
    add(memberId) {
        this.count++;
        this.members.push(memberId);
    }
